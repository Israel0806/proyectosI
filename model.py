...
class ParsecModel:
        ...
        if modeldatapath:
            ...
            if y_model is None:
                ...
                self.y_model = data_attach(self.predict(measure_detach['x']),
                                           measure_detach['dims'])
                self.error = mean_squared_error(measure_detach['y'],
                                                data_detach(self.y_model)['y'])
                self.errorrel = 100 * (self.error / self.measure.values.mean())
            else:
    ...
    def predict(self, x):
        ...
        :param x: array with number of cores and frequencies
                     or array of array.
        ...
        if self.modelFun: # DeSensi
            pred = phy_model.model(self.sol, x,  self.kwargs['f_min'], self.kwargs['min_exec_time'])
        else:
            pred = phy_model.model(self.sol, x)
        return pred
    ...
    def plot3D(self, train_points=None,
        ...
        def update_plot3D(idx):
            ...
            if color:
                ...
                if greycolor:
                    colormap = cm.Greys
                    surf1 = ax.plot_surface(Y, X, Z, label='Model',
                                            cmap=colormap,
