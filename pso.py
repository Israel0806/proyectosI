...
class Swarm:
    ...
    def __init__(self, lowervalues, uppervalues, parsecpydatafilepath=None,
        ...
        # CARGA MODELFUNC_EXAMPLE_INPUTSIZE_CORES.PY = la funcion de speedup de memory wall
        ...
        # A obj_wrapper, le pasamos definida la funcion objective_function
        # A obj_wrapper, le pasamos definida x_meas, y_meas y kwargs
        # Dejando al usuario ingresar el p de particulas
        ...
        for i, p in enumerate(self.particles):
            newfpos[i] = self.obj(p) # Llama a la objective_function
            constraint[i] = self.constr(p)
            if constraint[i]:
                bestfpos[i] = p.setfpos(newfpos[i]) # Actualiza la mejor posicion de esa particula
        self.bestparticle = deepcopy(self.particles[np.argmin(bestfpos)])

    @staticmethod
    def _obj_wrapper(func, x_meas, y_meas, p):
        """
        ...
        return func(p.pos, x_meas, y_meas)

    @staticmethod
    def _constraint_wrapper(func, x_meas, p):
        """
    ...
    def run(self):
        ...
        if self.threads > 1:
            mpool.terminate()
        print("Best pos")
        print(self.bestparticle.fpos)
        print(self.bestparticle.pos)
        print()
...
class Swarm:
    ...
    def run(self):
        ...
        while sm > 1e-8 and gbestmax < 50 and iteration < self.maxiter:
        #while iteration < self.maxiter:
            if self.verbosity > 1:
                print('Iteration: ', iteration+1,
                      ' - Error: ', self.bestparticle.bestfpos)
