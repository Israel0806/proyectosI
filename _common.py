...
def measures_split_train_test(measure, train_size, onlyPowerOfTwo = False):
    """
    ...
    # Check if application only runs with cores with power of two
    if onlyPowerOfTwo:
        msk = (m_detach['x'][:,1] & (m_detach['x'][:,1]-1) == 0)
        ind=np.where(msk)
        m_detach['x'] = m_detach['x'][ind]
        m_detach['y'] = m_detach['y'][ind]
